...
impl RelayPath {
    ...
    /// Relays using a specific sender, which implements
    /// [`relay_sender::Submit`].
    ...
    /// [`relay_sender::Submit`], which consists of either the tx
    /// hashes generated by the target chain (if [`Async`] sender)
    /// or the ibc events (if [`Sync`] sender).
    ///
    ...
    pub fn execute_schedule(&mut self) -> Result<RelaySummary, LinkError> {
        ...
        for od in dst_ods {
            summary.extend(self.relay_from_operational_data::<relay_sender::SyncSender>(od)?);
        }
